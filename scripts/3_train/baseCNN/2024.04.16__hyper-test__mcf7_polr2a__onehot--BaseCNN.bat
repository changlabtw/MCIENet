...
set input_folder=data/train/mcf7_polr2a
set out_folder=output/mcf7_polr2a/2024.04.16_cnn_hyper-test
set input_format=onehot

set "lr=0.001"
set "hidden_size=200, 300"
set "extractor_hidden_size=100, 200"
@REM set "extractor_hidden_size=300"
        ...
        for %%c in (%hidden_layer%) do (
            for %%d in (%bp_range_ls%) do (
                ...
                python train.py ^
                    ...
                    --output_folder %out_folder%/Lr%%a_CHs%%e_Hs%%b_Hl%%c_Bp%%d ^
                    --device gpu ^
                    --eval_freq 1 ^
